...
class Sinh(Operation):
    ...
    def _forward(self, x: Tensor) -> Tensor:
        """Forward pass of the hyperbolic sin operation.

        Args:
            x (Tensor): Input tensor.

        Returns:
            Tensor: Hyperbolic sin of the input tensor.
        """
        data = np.sinh(x.data)
    ...
    def _backward(self, grad: np.ndarray) -> Tuple[np.ndarray, ...]:
        """Gradient of the hyperbolic sin operation.
        ...
        grad_x = grad * np.cosh(self.inputs[0].data)
